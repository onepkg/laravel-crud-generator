...
if (! function_exists('auto_build_query')) {
    function auto_build_query(Builder $query, array $validated, array $customized = []) {
        $columns = collect(Schema::getColumns($query->getModel()->getTable()))
            ->map(function ($item) {
                return (array) $item;
            })
            ->keyBy('COLUMN_NAME')
        ...
        foreach ($validated as $column => $value) {
            if (isset($customized[$column]) && is_callable($customized[$column])) {
